...
public class ServerClockManager : MonoBehaviour
{
    ...
    public static ServerClockManager GetInstance()
    {
        if (Instance == null)
        {
            Instance = FindObjectOfType<ServerClockManager>();
            if (Instance == null)
                Debug.LogError("[ServerClockManager] 씬에 ServerClockManager가 없습니다!");
        }
        return Instance;
    }
    ...
    [SerializeField] private float randomStartMinSecondsBeforeTarget = 240f;
    [SerializeField] private float randomStartMaxSecondsBeforeTarget = 300f;
