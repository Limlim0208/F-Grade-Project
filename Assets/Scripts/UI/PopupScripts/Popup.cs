...
public class Popup : MonoBehaviour
{
    ...
    [SerializeField] protected ProfileOptionsPanel profileOptionsPanel;
    ...
    protected virtual void SetProfileOptions(PopupInfo info)
    {
        if (profileOptionsPanel == null) return;

        bool show = info.HasProfileOptions;
        profileOptionsPanel.gameObject.SetActive(show);

        if (show)
            profileOptionsPanel.Refresh();
    }


    public int GetSelectedProfileImageIndex()
    {
        return profileOptionsPanel != null ? profileOptionsPanel.GetSelectedIndex() : -1;
    }
